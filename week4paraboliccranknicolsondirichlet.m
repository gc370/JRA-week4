function [matrixuCNwithoutml,errormatrixCNwithoutml] = week4paraboliccranknicolsondirichlet(xbeginning,xend,tbeginning,tend,g1,g2,eta,numberofpointsinx,numberofpointsint,f,actualsol)
   ...
   matrixuCNwithoutml = zeros(numberofpointsint+1,numberofpointsinx+1);
   
   % ---- Insert first row (t=0) using eta (intial condition) ---- %
   
           for i = 1:numberofpointsinx+1
                   matrixuCNwithoutml(1,i) = eta(pointx(i)); 
           end        
   ...
   invertedmatrix = inv(matrixtobeinverted);
          ...
          for j = 2:numberofpointsint+1
                ...
                fvec(numberofpointsinx-1)= (dt*dx/2)*(f(pointx(numberofpointsinx),pointt(j-1)) + f(pointx(numberofpointsinx),pointt(j))) + (g2*(dt/dx));
                    ...
                    newresultsfornextiteration = invertedmatrix*((matrixm - ((dt/2)*matrixa))*(transpose(matrixuCNwithoutml(j-1,2:numberofpointsinx)) + fvec));        
                    
                    for i=1:numberofpointsinx-1
            
                        matrixuCNwithoutml(j,i+1) = newresultsfornextiteration(i);  
       
                    end
            
                matrixuCNwithoutml(j,1) = g1;
                matrixuCNwithoutml(j,numberofpointsinx+1) = g2;
          end 
  ...
  errormatrixCNwithoutml = abs(matrixuCNwithoutml - correctsols);
