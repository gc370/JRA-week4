function [matrixuexplicitwithoutml,errormatrixexplicitwithoutml] = week4parabolicexplicitdirichlet(xbeginning,xend,tbeginning,tend,g1,g2,eta,numberofpointsinx,numberofpointsint,f,actualsol)
   ...
   matrixuexplicitwithoutml = zeros(numberofpointsint+1,numberofpointsinx+1);
   
   %---- Insert first row (t=0) using eta (intial condition) ---- %
   
           for i = 1:numberofpointsinx+1
                   matrixuexplicitwithoutml(1,i) = eta(pointx(i)); 
           end        
   ...
   invm = inv(matrixm);
          ...
          for j = 2:numberofpointsint+1
                ...
                fvec(1)= dt*dx*f(pointx(2),pointt(j-1)) + (g1*(dt/dx)) ;
                fvec(numberofpointsinx-1)= dt*dx*f(pointx(numberofpointsinx),pointt(j-1)) + (g2*(dt/dx));

                    for i = 2: numberofpointsinx-2 
                        fvec(i) = dt*dx*f(pointx(i+1),pointt(j-1));
                    end
           
                newresultsfornextiteration = (invm)*((matrixm - (dt*matrixa))*(transpose(matrixuexplicitwithoutml(j-1,2:numberofpointsinx)) + fvec));        
   
                    for i=1:numberofpointsinx-1
            
                        matrixuexplicitwithoutml(j,i+1) = newresultsfornextiteration(i);  
       
                    end
            
                matrixuexplicitwithoutml(j,1) = g1;
                matrixuexplicitwithoutml(j,numberofpointsinx+1) = g2;
          end 
  ...
  errormatrixexplicitwithoutml = abs(matrixuexplicitwithoutml - correctsols);
