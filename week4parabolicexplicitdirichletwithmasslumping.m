function [matrixuexplicit,errormatrixexplicit] = week4parabolicexplicitdirichletwithmasslumping(xbeginning,xend,tbeginning,tend,g1,g2,eta,numberofpointsinx,numberofpointsint,f,actualsol)
   ...
   matrixuexplicit = zeros(numberofpointsint+1,numberofpointsinx+1);
   
   % ---- Insert first row (t=0) using eta (intial condition) ---- %
   
           for i = 1:numberofpointsinx+1
                   matrixuexplicit(1,i) = eta(pointx(i)); 
           end        
   ...
   invm = inv(matrixm);
          ...
          for j = 2:numberofpointsint+1
                ...
                newresultsfornextiteration = invm*((matrixm - dt*matrixa)*(transpose(matrixuexplicit(j-1,2:numberofpointsinx)) + fvec));        
   
                    for i=1:numberofpointsinx-1
            
                        matrixuexplicit(j,i+1) = newresultsfornextiteration(i);  
       
                    end
            
                matrixuexplicit(j,1) = g1;
                matrixuexplicit(j,numberofpointsinx+1) = g2;
          end 
  ...
  errormatrixexplicit = abs(matrixuexplicit - correctsols);
